...
export default async function BlogPostPL(props: {
	params: Promise<{ slug: string }>;
}) {
	const params = await props.params;
	const post = getPostBySlug("pl", params.slug);
	...
	const crumbs = breadcrumbsJsonLd(SITE_URL, [
		...
		{ name: "Poradnik", path: "/pl/poradnik" },
		{
			...
			path: `/pl/poradnik/kategoria/${post.categorySlug}`,
		},
					...
					<div className="sticky top-24">
						<div className="rounded-xl bg-slate-50 p-6 border border-slate-100">
							...
							<p className="text-sm text-slate-600 mb-4">
								Nasz zespół chętnie pomoże.
							</p>
							<a
								href="tel:+436641234567"
								className="block w-full rounded-lg bg-red-600 px-4 py-2 text-center text-sm font-bold text-white hover:bg-red-700"
							>
								WhatsApp / Telefon
							</a>
						</div>
