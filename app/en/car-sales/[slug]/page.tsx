...
export default async function VehicleDetailPage({ params }: Props) {
    ...
    // Helpers
    const numberLocale = locale === 'de' ? 'de-AT' : (locale === 'pl' ? 'pl-PL' : 'en-US');
    const currencyFormatter = new Intl.NumberFormat(numberLocale, { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 });
    const numberFormatter = new Intl.NumberFormat(numberLocale);

    const formatPrice = (val?: number) => val ? currencyFormatter.format(val) : '';
        ...
        <div className="space-y-10 pb-16 font-sans">
            ...
            <section className="bg-gradient-to-b from-white to-gray-50 py-8 border-b border-gray-200">
                <div className="mx-auto max-w-7xl px-4">
                    <Link href={getRoute('vehicles', locale)} className="text-sm font-medium text-brand-primary mb-6 inline-flex items-center">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8">
                        <div className="flex-1">
                            ...
                            {vehicle.subtitle && (
                                <p className="mt-2 text-xl text-gray-500 font-light">{vehicle.subtitle}</p>
                            )}
                        ...
                        <div className="lg:text-right bg-white p-5 rounded-2xl shadow-sm border border-gray-100 lg:bg-transparent lg:p-0 lg:shadow-none lg:border-none min-w-[280px]">
                            {vehicle.priceStatus === 'Auf Anfrage' || vehicle.price === 0 ? (
                                <span className="text-3xl font-bold text-gray-900">{t.vehicleDetail.onRequest}</span>
                            ) : (
                                <>
                                    <span className="text-4xl font-extrabold text-[#d32f2f] block">
                                        {formatPrice(vehicle.price)}
                                    </span>
                                    {(vehicle.priceNetto ?? 0) > 0 && vehicle.price !== vehicle.priceNetto && (
                                        <span className="text-sm text-gray-500 block mt-1">
                                            {t.vehicleDetail.priceNet}: {formatPrice(vehicle.priceNetto!)}
                                        </span>
                                    )}
                                </>
                            )}
                            ...
                            <div className="text-sm text-gray-600 mt-2 space-y-0.5">
                                {vehicle.priceStatus === 'Verhandelbar' && (
                                    <span className="block font-medium text-brand-accent">{t.vehicleDetail.negotiable}</span>
                                )}
                                {vehicle.vatRate && vehicle.vatRate > 0 ? (
                                    <span className="block">{t.vehicleDetail.vat}: {formatNumber(vehicle.vatRate)}%</span>
                                ) : (
                                    vehicle.vatText && vehicle.vatText !== '0' && <span className="block">{vehicle.vatText}</span>
                                )}
                                {isSold && <span className="block text-red-600 font-medium mt-1">({t.vehicleDetail.sold})</span>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="mx-auto max-w-7xl px-4">
                <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
                    ...
                    <div className="space-y-12 min-w-0">
                        ...
                        <div className="space-y-4">
                            ...
                            {(() => {
                                ...
                                if (vehicle.image && !vehicle.image.includes('placeholder')) {
                                    ...
                                    allImages.push({ url: vehicle.image, thumb: vehicle.image, alt: 'Main Image' });
                                }
                                ...
                                return <VehicleGallery images={allImages} title={vehicle.title} />;
                            })()}
                        </div>

                        {/* MOBILE ONLY: Technical Specs immediately after gallery */}
                        <VehicleSpecs vehicle={vehicle} t={t} locale={locale} className="lg:hidden" />

                        {/* Video */}
                        {vehicle.videoUrl && (
                            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                                <h3 className="text-lg font-bold mb-3 text-gray-900">{t.vehicleDetail.video}</h3>
                                <a href={vehicle.videoUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center px-5 py-2.5 bg-[#d32f2f] text-white font-semibold rounded-lg hover:bg-red-700 transition shadow-sm">
                                    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20"><path d="M10 0a10 10 0 100 20 10 10 0 000-20zm-2 14.5v-9l6 4.5-6 4.5z" /></svg>
                        ...
                        {/* Description */}
                        <div className="space-y-4">
                            <h3 className="text-2xl font-bold text-gray-900 border-b pb-2">{t.vehicleDetail.description}</h3>
                            <div
                        ...
                        {(vehicle.conditionDescription || vehicle.conditionDetail) && (
                            <div className="bg-[#fff3e0] p-6 rounded-2xl border border-orange-100 shadow-sm text-sm">
                                ...
                                {vehicle.conditionDescription && (
                                    <p className="text-orange-900 mb-3 whitespace-pre-line">{vehicle.conditionDescription}</p>
                                )}
                                {vehicle.conditionDetail && (
                                    <p className="text-orange-900 whitespace-pre-line border-t border-orange-200 pt-3">{vehicle.conditionDetail}</p>
                                )}
                            </div>
                        )}
                        ...
                        {vehicle.equipmentList && vehicle.equipmentList.length > 0 && (
                            <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
                                <h3 className="text-xl font-bold mb-6 text-gray-900">{t.vehicleDetail.equipment}</h3>
                    ...
                    <aside className="space-y-6">
                                ...
                                {vehicle.vin && (
                                    <div className="mt-4 pt-3 border-t border-gray-200">
                                        ...
                                        <div className="relative group cursor-pointer">
                                            ...
                                            <a
                                                href={`https://wa.me/436641261735?text=${encodeURIComponent(`Hello, I am interested in the VIN of the vehicle: ${vehicle.title}`)}`}
                                                target="_blank"
                        ...
                        <div className="rounded-2xl bg-brand-primary p-6 text-white shadow-lg">
                            <h3 className="text-xl font-bold mb-3">{t.vehicleDetail.interested}</h3>
                            <p className="mb-6 opacity-90 leading-snug">
                                {'Call us or visit us personally in Sollenau (Industriestraße 1).'}
                            </p>
                            <div className="space-y-3">
                                <Link href={contactHref} className="block w-full rounded-xl bg-white text-brand-primary py-3.5 text-center font-extrabold transition hover:bg-gray-100 uppercase tracking-wide text-sm">
                                    {t.vehicleDetail.view}
                                </Link>
                                <Link href={towingHref} className="block w-full rounded-xl border-2 border-white/20 py-3.5 text-center text-white font-bold transition hover:bg-white/10 uppercase tracking-wide text-sm">
