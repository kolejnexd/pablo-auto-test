...
import { LOCATION_PAGES, LocationPageConfig } from '../../lib/data/location-pages';
...
const getLocationPageConfig = cache((slug: string): LocationPageConfig | undefined => {
...
export async function generateMetadata({ params }: { params: { locationSlug: string } }) {
    ...
    return {
        ...
        openGraph: {
            ...
            locale: activeLocale === 'de' ? 'de_AT' : activeLocale === 'pl' ? 'pl_PL' : 'en_US',
...
export default function LocationPage({ params }: { params: { locationSlug: string } }) {
    ...
    return (
        <LocationPageTemplate config={config} />
    );
}
