...
import { getRoute, RouteKey } from '../lib/routes';
...
const staticRoutes: { key: RouteKey; priority: number; changeFrequency: SitemapEntry['changeFrequency'] }[] = [
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    const addEntry = (entry: SitemapEntry) => {
        ...
        const url = entry.url.endsWith('/') && entry.url.length > SITE_URL.length + 1
            ? entry.url.slice(0, -1)
            : entry.url;
    ...
    return Array.from(entriesMap.values()).sort((a, b) => a.url.localeCompare(b.url));
}
