import { Vehicle, WPVehicle, VehicleListing, VehicleListingResponse } from './types';

const WP_API_URL = process.env.NEXT_PUBLIC_WORDPRESS_URL || 'https://panel.pablo-auto.at';
...
function mapWpToVehicle(wpItem: WPVehicle): Vehicle {
  ...
  if (acf.fahrzeug_baujahr) {
    year = parseInt(acf.fahrzeug_baujahr);
  } else if (acf.erstzulassung) {
    year = parseInt(String(acf.erstzulassung).substring(0, 4));
  }

  const imageUrl = wpItem._embedded?.['wp:featuredmedia']?.[0]?.source_url || '/assets/placeholder-car.webp';

  // Mapowanie galerii
  const gallery = (Array.isArray(acf.fahrzeug_galerie) ? acf.fahrzeug_galerie : [])?.map(img => ({
    url: img.url,
  ...
  let priceGross = Number(acf.fahrzeug_preis_brutto) || Number(acf.fahrzeug_preis) || Number(acf.preis) || 0;
  ...
  return {
    ...
    power: acf.fahrzeug_leistung || (acf.leistung_ps ? `${acf.leistung_ps} PS` : ''),
    ...
    towingCapBraked: acf.fahrzeug_trailer_braked || acf.fahrzeug_anhaengelast_gebremst,
    towingCapUnbraked: acf.fahrzeug_trailer_unbraked || acf.fahrzeug_anhaengelast_ungebremst,
...
export async function getLatestVehicles(count: number = 3): Promise<Vehicle[]> {
...
export async function getSitemapVehicles(): Promise<{ slug: string; modified: string }[]> {
...
function mapWpToVehicleListing(wpItem: WPVehicle): VehicleListing {
  ...
  // Year logic
  let year = 2024;
  if (acf.fahrzeug_baujahr) year = parseInt(acf.fahrzeug_baujahr);
  else if (acf.erstzulassung) year = parseInt(String(acf.erstzulassung).substring(0, 4));
  ...
  // Power logic
  const power = acf.fahrzeug_leistung || (acf.leistung_ps ? `${acf.leistung_ps} PS` : 'k.A.');
...
export async function getVehicleListings(page: number = 1, perPage: number = 24): Promise<VehicleListingResponse> {
