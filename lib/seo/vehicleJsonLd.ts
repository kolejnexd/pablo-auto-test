import {
	...
	Vehicle as SchemaVehicle,
	type WithContext,
} from "schema-dts";
import { type Locale, getRoute } from "../routes";
...
function formatDate(dateStr?: string | number): string | undefined {
	...
	if (
		...
		(typeof dateStr === "string" && dateStr.length === 4)
	) {
		return `${dateStr}`;
	}
...
export function buildVehicleJsonLd(
	...
	const vehiclesRoute = vehiclesPath.startsWith("/")
		? vehiclesPath
		: `/${vehiclesPath}`;
	...
	const schema: Car = {
		...
		image: vehicle.image
			? [vehicle.image, ...(vehicle.gallery?.map((g) => g.url) || [])]
			: undefined,
		url: vehicleUrl,
		brand: {
			"@type": "Brand",
		...
		dateVehicleFirstRegistered: vehicle.firstRegistration
