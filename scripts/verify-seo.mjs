...
const ROUTES = [
	// Homepages
	{
		...
		expectedStatus: 200,
		checks: [
			'<link rel="canonical" href="https://pablo-auto.at',
			'hreflang="pl"',
			'hreflang="en"',
		],
	},
	{
		...
		expectedStatus: 200,
		checks: [
			'<link rel="canonical" href="https://pablo-auto.at/pl',
			'hreflang="de"',
		],
	},
	{
		...
		expectedStatus: 200,
		checks: [
			'<link rel="canonical" href="https://pablo-auto.at/en',
			'hreflang="de"',
		],
	},
	...
	// Specific Redirects
	{
		path: "/autohandel",
		expectedStatus: 308,
		expectedRedirect: "/autohandel-gebrauchtwagen",
	},
];
...
async function checkRoute(route) {
	try {
		...
		if (res.status !== route.expectedStatus) {
			console.error(
				`[FAIL] ${route.path}: Expected status ${route.expectedStatus}, got ${res.status}`,
			);
			return false;
		}
		...
		if (route.expectedStatus >= 300 && route.expectedStatus < 400) {
			...
			if (!location || !location.endsWith(route.expectedRedirect)) {
				console.error(
					`[FAIL] ${route.path}: Expected redirect to ${route.expectedRedirect}, got ${location}`,
				);
				return false;
			}
		...
		if (route.checks) {
			for (const check of route.checks) {
				...
				if (!lowerText.includes(lowerCheck)) {
					...
					console.error(
						`[FAIL] ${route.path}: Missing expected content "${check}"`,
					);
