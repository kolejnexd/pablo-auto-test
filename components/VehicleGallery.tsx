                ...
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x">
                    {images.map((img, idx) => (
                        <button
                            ...
                            className={`relative flex-shrink-0 w-24 aspect-[4/3] rounded-lg overflow-hidden transition-all duration-300 snap-start ${selectedIndex === idx
                        ...
                        <div className="absolute bottom-6 flex gap-2 overflow-x-auto max-w-[90vw] p-2 bg-black/50 rounded-xl backdrop-blur-sm">
                            {images.map((img, idx) => (
                                <button
                                    ...
                                    className={`relative w-16 aspect-square rounded-md overflow-hidden transition ${selectedIndex === idx ? 'opacity-100 ring-2 ring-white' : 'opacity-40 hover:opacity-100'
                                        }`}
                                >
                                    <Image src={img.thumb || img.url} alt="" fill className="object-cover" />
                                </button>
                            ))}
                        </div>
