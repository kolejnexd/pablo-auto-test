...
import { CATEGORY_NAMES, ClusterKey } from "../../lib/blog/config";
...
export default function PostCard({ post }: { post: BlogPost }) {
    ...
    const READ_MORE: Record<string, string> = { de: "Mehr lesen", pl: "Czytaj więcej", en: "Read more" };
    ...
    return (
        <article className="group flex flex-col h-full bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-slate-100/50">
            {post.heroImage ? (
                <Link href={post.url} className="relative aspect-[16/9] block overflow-hidden">
                    <Image
                        src={post.heroImage}
                        alt={post.title}
                        fill
            ...
            <div className="flex flex-col flex-grow p-6">
                <div className="flex items-center space-x-2 text-xs font-medium text-slate-500 mb-2">
                    <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded-full">{categoryName}</span>
                </div>
                <div className="flex items-center space-x-2 text-xs font-medium text-slate-400 mb-3">
                    <time dateTime={post.date}>{new Date(post.date).toLocaleDateString(locale === 'de' ? 'de-AT' : locale === 'pl' ? 'pl-PL' : 'en-GB')}</time>
                    <span>•</span>
