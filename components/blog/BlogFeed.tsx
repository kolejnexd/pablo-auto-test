...
const LABELS = {
	...
	en: {
		searchPlaceholder:
			"What are you looking for? (e.g. breakdown, buying car...)",
		...
		<main className="mx-auto max-w-6xl px-4 py-10">
			...
			<header className="mb-12 text-center relative">
				...
				<h1 className="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl mb-4">
					{locale === "de"
						? "Ihr Experten-Ratgeber rund um Mobilität"
						: locale === "pl"
							? "Ekspercki poradnik mobilności Pablo e.U."
							: "Pablo e.U. Mobility Guide"}
				</h1>
				...
				>
					{categories.map((key) => {
						...
						const path =
							locale === "de" ? `/blog/${slug}` : `/${locale}/blog/${slug}`;
						return (
							<Link
								key={key}
								href={path}
			...
			<section className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				{filteredPosts.length > 0 ? (
					filteredPosts.map((p) => <PostCard key={p.url} post={p} />)
				) : (
