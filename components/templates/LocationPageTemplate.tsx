...
import type { LocationPageConfig } from "../../lib/data/location-pages";
...
export default function LocationPageTemplate({ config }: Props) {
	...
	const ctaRoute =
		config.serviceCategory === "towing"
			? getRoute("contact", locale) // Urgency -> Contact/Call
			: config.serviceCategory === "carSales"
				? getRoute("carSales", locale)
				: config.serviceCategory === "rental"
					? getRoute("rental", locale)
		...
		<div className="flex flex-col min-h-screen">
				...
				<div className="relative mx-auto max-w-4xl text-center space-y-6">
					...
					<div className="flex flex-wrap justify-center gap-4 pt-4">
						{config.trustSignals.map((signal, idx) => (
							<div
								key={idx}
								className="flex items-center gap-2 bg-white rounded-full px-4 py-2 border border-slate-200 shadow-sm text-slate-700"
							>
						...
						<div className="prose prose-lg text-gray-700">
							<h2 className="text-3xl font-bold text-brand-primary mb-6">
								{locale === "pl"
									...
									: locale === "en"
										? "Why choose us?"
										: `Warum Pablo e.U. in ${config.geoEntity}?`}
							</h2>
							...
							<ul className="not-prose space-y-3 mt-6">
								...
								<li className="flex gap-3">
									...
									<span>
										{locale === "pl"
											? "Dojazd w 30 minut (Sollenau/A2)"
											: locale === "en"
												? "Arrival in 30 mins (Sollenau/A2)"
												: "Anfahrt in 30 Min (Sollenau/A2)"}
									</span>
								</li>
								<li className="flex gap-3">
									<span className="text-brand-accent font-bold">➢</span>
			...
			<JsonLd
				data={{
					...
					"@type":
						config.serviceCategory === "carSales" ? "AutoDealer" : "AutoRepair",
					name: config.h1,
