...
export default function FaqAccordion({ items }: { items: { question: string; answer: string; }[] }) {
  const [active, setActive] = useState<string | null>(items[0]?.question ?? null);
  return (
    <div className="divide-y divide-gray-200 rounded-2xl bg-white shadow-soft">
      {items.map((item) => (
        <button
          ...
          onClick={() => setActive((prev) => (prev === item.question ? null : item.question))}
        ...
        >
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <span className="text-xl text-brand-primary">{active === item.question ? '−' : '+'}</span>
          </div>
          {active === item.question ? (
            <div className="px-6 pb-4 text-sm text-gray-700 leading-relaxed">{item.answer}</div>
          ) : null}
        </button>
      ))}
    </div>
